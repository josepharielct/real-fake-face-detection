...
class AlexNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc(x) 
        return x
google_drive_file_id = '1VSCTmO1dvJoZJnTHsfzI7sCEIRl6kT-R'
url = f'https://drive.google.com/uc?id={google_drive_file_id}'
output_file_path = 'model.pkl'
gdown.download(url, output_file_path, quiet=False)
model = torch.load(output_file_path,map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
